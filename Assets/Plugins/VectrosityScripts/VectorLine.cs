...
public class VectorLine {
	public static string Version () {
		return "Vectrosity version 3.1.2";
	}
	...
	public int sortingLayerID {
		get {return vectorObject.GetComponent<Renderer>().sortingLayerID;}
		set {
			vectorObject.GetComponent<Renderer>().sortingLayerID = value;
		}
	}
	public int sortingOrder {
		get {return vectorObject.GetComponent<Renderer>().sortingOrder;}
		set {
			if (value < -32768 || value > 32767) {
				Debug.LogError ("sortingOrder out of range");
				return;
			}
			vectorObject.GetComponent<Renderer>().sortingOrder = value;
		}
	}
...
//		m_mesh.MarkDynamic();	// Apparently buggy on some platforms; makes little difference in speed anyway
	...
	public void SetWidth (float width) {
		SetWidth (width, 0, pointsLength, true);
	}
	...
	private void SetWidth (float width, int startIndex, int endIndex, bool setAll) {
		...
		if (m_lineWidths.Length == 1) {
			if (setAll) {
				...
				m_lineWidths = new float[pointsLength];
	...
	private void SetCollider (bool convertToWorldSpace) {
		...
		int min = (drawStart > minDrawIndex)? drawStart : minDrawIndex;
...
//		BoxCollider2D capBox = new BoxCollider2D();
//			capBox.center = Vector2.zero;
//		capBox.size = new Vector2(width, height);
	...
	private void ZeroVertices (int startIndex, int endIndex) {
		...
		if (m_1pixelLine) {
			if (m_continuous) {
				...
				if (endIndex*2 > m_vertexCount) {
					endIndex--;
				}
				for (int i = startIndex; i < endIndex; i++) {
					m_lineVertices[index  ] = v3zero;
					m_lineVertices[index+1] = v3zero;
					index += 2;
				}
			}
			else {
